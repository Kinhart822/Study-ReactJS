...
8. [Phần 8: Routing với React Router](#-phần-8-routing-với-react-router)
...
## 🚦 Phần 8: Routing với React Router

### 20. React Router (v6/v7)
...
React-Router là một thư viện định tuyến (routing) tiêu chuẩn trong React. Nó giữ cho giao diện của ứng dụng đồng bộ với URL trên trình duyệt. React-Router cho phép chuyển đổi "luồng giao diện" trong ứng dụng theo kiến trúc **Single Page Application (SPA)** – chuyển trang lập tức, mượt mà và **không bị reload lại trang browser**.

#### 1. Các Component Bao Bọc Cơ Bản:
- `<BrowserRouter>`: Là thành phần gốc bọc toàn bộ ứng dụng, sử dụng HTML5 History API để theo dõi URL.
- `<Routes>`: Là vùng chứa, duyệt qua các `<Route>` con bên trong và chọn ra giao diện khớp với URL hiện tại nhất để hiển thị.
- `<Route>`: Gồm 2 thuộc tính chính là `path` (đường dẫn URL) và `element` (giao diện component hiển thị tương ứng).

#### 2. So sánh BrowserRouter & HashRouter
- **BrowserRouter:** (Được dùng phổ biến nhất) Sử dụng History API trong HTML5 để ghi chú hành vi. URL sẽ sạch đẹp dạng: `domain.com/about`.
- **HashRouter:** Sử dụng hash (`#`) của URL (`window.location.hash`) để ghi nhớ. Trang web sẽ có dạng: `domain.com/#/about`. Thường dùng trong các trường hợp server cũ hoặc file tĩnh không hỗ trợ rewrite URL.

#### 3. Điều hướng: `<Link>` vs `<NavLink>`
Trong HTML truyền thống thường dùng thẻ `<a href="...">` để chuyển trang, nhưng trong React nên dùng `<Link>` hoặc `<NavLink>` để **ngăn việc tải lại (reload)** toàn bộ trang gốc.

- **`<Link to="/about">`**: Chuyển hướng âm thầm (Thay thế `<a href>`). 
- **`<NavLink to="/about">`**: Phiên bản nâng cấp của `<Link>`, chuyên dùng cho làm **Menu Navigation (Thanh điều hướng)**. Giúp biết được đường link này có đang khớp (active) với trang hiện tại hay không để đổi màu/giao diện nút nhấn.

**Ví dụ dùng NavLink với Tailwind:**
```jsx
<NavLink 
  to="/about" 
...
#### 4. Hook `useNavigate()` (Chuyển trang bằng Code)
Hook giúp tự động điều hướng sang trang khác bằng JavaScipt thay vì chờ user click. Rất hữu dụng khi cần **tự động chuyển trang sau khi thực thi hàm** (vd: sau khi gọi API Login thành công thì tự đẩy vào Dashboard).
...
function Login() {
  ...
  const handleLogin = () => {
    ...
    navigate("/"); // Chuyển thẳng về trang chủ luôn
  };
}
```

#### 5. Truyền Tham số URL (Params & Query)
Để truyền dữ liệu giữa các trang, ta dùng URL parameter:

- **1. URL Params (`/:id`):** Dùng để truy xuất dữ liệu động. (Vd: User Profile, Chi tiết 1 sản phẩm).
  ```jsx
  // Trong App.jsx khai báo: <Route path="/user/:userId" element={<UserProfile />} />
  
  // Lấy dữ liệu:
  import { useParams } from "react-router";
  const { userId } = useParams(); // URL là /user/123 -> userId = "123"
  ```
- **2. Query Params (`?key=value`):** Dùng cho việc Lọc (filter), Tìm kiếm (search), Phân trang (pagination).
  ```jsx
  import { useSearchParams } from "react-router";
  const [searchParams, setSearchParams] = useSearchParams();
  const keyword = searchParams.get("query"); // URL là /search?query=react -> keyword = "react"
  ```
